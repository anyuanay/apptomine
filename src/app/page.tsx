...
import { Zap, Github, ArrowRight, Download, Terminal, FolderOpen, MessageSquare, Search, Wand2, Loader2, X, ChevronUp, ImageIcon, Moon, FileCode, Gamepad2, Sparkles } from 'lucide-react';
...
import { filterApps, DISCOVERY_MODES } from '@/lib/categories';
...
export default function HomePage() {
  ...
  const searchGitHub = useCallback(async (q: string) => {
    ...
    setGhResults(null);
    try {
      const cat = category === 'all' ? 'creative' : category;
      const res = await fetch(`/api/crawl?query=${encodeURIComponent(trimmed)}&category=${cat}&perPage=12&maxStars=500`);
  ...
  const filtered = useMemo(
    () => filterApps(ALL_APPS, category, search, discoveryMode),
    [search, category, discoveryMode]
  );

  const counts = useMemo(() => {
    const base = filterApps(ALL_APPS, 'all', search, discoveryMode);
  ...
  const discoveryModeIcons: Record<DiscoveryMode, React.ReactNode> = {
    'all': <Sparkles className="h-3.5 w-3.5" />,
    'dormant-gems': <Moon className="h-3.5 w-3.5" />,
    'single-file': <FileCode className="h-3.5 w-3.5" />,
    'jam-survivors': <Gamepad2 className="h-3.5 w-3.5" />,
    'tiny-and-complete': <Zap className="h-3.5 w-3.5" />,
  };
    ...
    <div className="min-h-screen bg-slate-950">
      ...
      <section id="apps" className="px-4 py-12">
        <div className="mx-auto max-w-7xl">
          ...
          {/* Discovery Modes */}
          <div className="mb-4">
            <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-slate-500">Discovery Mode</p>
