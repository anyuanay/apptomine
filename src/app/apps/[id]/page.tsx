...
function getClaudeCodeSteps(app: App): TutorialStep[] {
  ...
  return [
    ...
    },
    {
      ...
      description: `Before customizing, get the app running locally so you can see your changes in real time. Follow the setup and run instructions in the README on the app's GitHub page — look for sections like "Getting Started", "Installation", or "Running locally".`,
      note: `The GitHub page for this app is: ${app.githubUrl}`,
    },
    {
      step: 4,
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div>
              <div className="mb-3 flex flex-wrap items-center gap-3">
                ...
                {app.singleFile && (
                  <span className="flex items-center gap-1 rounded-full bg-cyan-500/15 px-2.5 py-0.5 text-xs text-cyan-400">
            ...
            <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-5">
              ...
              <p className="text-sm text-slate-300 leading-relaxed italic">{app.spark}</p>
            </div>
          ...
          <aside className="space-y-4">
            ...
            <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-5">
              <h3 className="mb-4 flex items-center gap-2 text-sm font-semibold text-amber-400">
                <Flame className="h-4 w-4" />
                Seed Score: {app.seedScore.total}
              </h3>
              <div className="space-y-2">
                {[
                  { label: 'Idea Novelty', value: app.seedScore.ideaNovelty, max: 10 },
                  { label: 'Size Fit', value: app.seedScore.sizeFit, max: 10 },
                  { label: 'Scope Clarity', value: app.seedScore.scopeClarity, max: 10 },
                  { label: 'README Clarity', value: app.seedScore.readmeClarity, max: 10 },
                ].map(({ label, value, max }) => (
                  <div key={label}>
                ...
                <div className="mt-3 flex flex-wrap gap-1.5 text-xs">
                  {app.seedScore.singleAuthorBonus > 0 && (
                    <span className="rounded bg-emerald-500/15 px-1.5 py-0.5 text-emerald-400">+{app.seedScore.singleAuthorBonus} single author</span>
                  )}
                  {app.seedScore.dormancyBonus > 0 && (
                    <span className="rounded bg-amber-500/15 px-1.5 py-0.5 text-amber-400">+{app.seedScore.dormancyBonus} dormant</span>
                  )}
                  {app.seedScore.singleFileBonus > 0 && (
                    <span className="rounded bg-cyan-500/15 px-1.5 py-0.5 text-cyan-400">+{app.seedScore.singleFileBonus} single file</span>
                  )}
                  {app.seedScore.genericClonePenalty < 0 && (
                    <span className="rounded bg-red-500/15 px-1.5 py-0.5 text-red-400">{app.seedScore.genericClonePenalty} generic</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-white/10 bg-white/5 p-5">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm text-slate-500">Lines of Code</span>
                  <span className="text-sm font-medium text-white">{app.loc.toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-slate-500">Authors</span>
                  <span className="text-sm font-medium text-white">{app.authorCount}</span>
                </div>
                <div className="flex items-center justify-between">
